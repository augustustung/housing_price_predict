...
const run = async () => {
  ...
  // renderPredictions(trueValues, null, lmPreds);
  predictFromUserInput(linearModel, features, categoricalFeatures, trueValues, xTest)
};

function predictFromUserInput(linearModel, features, categoricalFeatures, trueValues, xTest) {
  const form = document.getElementById('form1');

  form.addEventListener('submit', async (e) => {
    e.preventDefault();
    const listValues = form
    const dataFromUser = [{
      overallQual: listValues.OverallQual,
      grLivArea: listValues.GrLivArea,
      garageCars: listValues.GarageCars,
      totalBsmtSF: listValues.TotalBsmtSF,
      fullBath: listValues.FullBath,
      yearBuilt: listValues.YearBuilt
    }]
    const [none1, usrVal, none2, none3] = createDataSets(
      dataFromUser,
      features,
      categoricalFeatures,
      0.1
    );
    const arrayDraw = xTest.concat(usrVal)
    const predict = linearModel.predict(arrayDraw).dataSync();
    renderPredictions(trueValues, null, predict);
    document.getElementById('last-price').innerHTML = `Giá dự đoán: ${predict[predict.length-1]} USD`;
    setTimeout(() => {
